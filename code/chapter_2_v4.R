...
S.time.state.stock <- list(formula = ~stratum + stock)

p.fixed <- list(formula = ~fix)

Psi.state.tostate.time.stock.sex <- list(formula = ~stratum + tostratum + stock + sex)
...
Psi <- get.real(coho.mod, 'Psi', vcv = T)
Psi.values <- Psi$estimates
fu <- TransitionMatrix(Psi.values[Psi.values$time == 1 & Psi.values$sex == 'F' & Psi.values$stock == 'Unalakleet',])

S <- get.real(coho.mod, 'S', vcv = T)
S.values <- S$estimates

fu[1,] <- fu[1,] * S.values[S.values$stratum == 'A' & S.values$sex == 'F' & S.values$time == '1' 
        & S.values$stock == 'Unalakleet', 3]
fu[2,] <- fu[2,] * S.values[S.values$stratum == 'B' & S.values$sex == 'F' & S.values$time == '1' 
                  & S.values$stock == 'Unalakleet', 3]
fu[3,] <- fu[3,] * S.values[S.values$stratum == 'X' & S.values$sex == 'F' & S.values$time == '1' 
                  & S.values$stock == 'Unalakleet', 3]
