...
### psi 
psi.state.sex <- list(formula = ~stratum + tostratum + sex)

# Make model ####

mod1<- mark(coho.proc, coho.ddl, model.parameters=list(S = S.state.sex, 
                                                            Psi = psi.state.sex, 
                                                            p = p.fix)) 
...
ms.results$S.time.p.fixed.Psi.state.tostate.sex$results$real
