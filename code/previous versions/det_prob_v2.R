...
ggplot(data = r7.group, aes(x = depth, y = prop)) +
  geom_point() +
...
r1.group$Array <- 'Bald Head'
r2.group$Array <- 'Point Dexter'
r3.group$Array <- 'Cape Denbeigh'
r4.group$Array <- 'Junction Creek'
r5.group$Array <- 'Blueberry Creek'
r6.group$Array <- 'Point Creek'
r7.group$Array <- 'Black Point'

r2020 <- rbind(r1.group, r2.group, r3.group, r4.group, r5.group, r6.group, r7.group)
...
ggplot(data = r7.group, aes(x = as.numeric(hour.seq), y = prop, col = as.factor(dist))) +
  ...
  scale_x_continuous(breaks = c(0, 500, 1000, 1500),
                     labels = c(r7.group[first(which.min(as.numeric(r7.group$hour.seq))), 2],
                                r7.group[first(which(as.numeric(r7.group$hour.seq) == 500)), 2],
                                r7.group[first(which(as.numeric(r7.group$hour.seq) == 1000)), 2],
                                r7.group[first(which.max(as.numeric(r7.group$hour.seq))), 2])) +
  scale_y_continuous(limits = c(0,1), expand = c(0,0)) +
  theme(panel.spacing.x = unit(1, "lines"))
...
r2021 <- rbind(r1.group, r2.group, r3.group, r4.group, r5.group, r6.group, r7.group)
