...
#detstockcoho20 <- filter(detstockcoho[,c(1:5, 7)], year(ymd.hms) == 2020)

### And let's convert 'stock' to a friendlier label
table(detstockcoho$stock)

detstockcoho$stock <- ifelse(detstockcoho$stock %in% c(4, 'N', 'S'), 'Transitory', 
       ifelse(detstockcoho$stock == 5, 'Shaktoolik', 'Unalakleet')) 
...
detcoho20 <- subset(detsum, tag.ID %in% detstockcoho$tag.ID)
...
det.list <- split(detstockcoho, f = detstockcoho$tag.ID)
...
# take out multiple river fish ####

movementnomultriv <- movement[which(!(movement$tag.ID %in% cohomultriv$tag.ID)),]
write.csv(movement, 'data/movement/movementnomultriv.csv', row.names = F)

table(movementnomultriv$stock)
moveskk <- movementnomultriv[which(movementnomultriv$stock == 'Shaktoolik'),]
length(unique(moveskk$tag.ID))
